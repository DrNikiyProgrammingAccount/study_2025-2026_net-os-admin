...
В открытом файле конфигурации /etc/samba/smb.conf мы вносим изменения в раздел global, задавая имя рабочей группы NSANDRYUSHIN-NET. В конце файла мы добавляем новую секцию sambashare, в которой описываем путь к каталогу, комментарий и список пользователей, имеющих право на запись (рис. [-@fig:003]).
...
В файле /etc/samba/smb.conf на клиенте нам нужно привести рабочую группу в соответствие с сервером. Мы меняем значение параметра workgroup на NSANDRYUSHIN-NET. Остальные параметры можно оставить по умолчанию для базового подключения (рис. [-@fig:015]).
...
Для проверки возможности монтирования создаем точку /mnt/samba. Сначала пробуем смонтировать ресурс вручную командой mount, передавая параметры пользователя и прав. После успешного монтирования создаем файл nsandryushin@client.txt в удаленной папке, чтобы подтвердить права на запись, а затем размонтируем ресурс (рис. [-@fig:017]).
...
Убеждаемся в успешном автомонтировании после перезагрузки. Вывод команды ls подтверждает, что в общем ресурсе находятся файлы nsandryushin@client.txt и nsandryushin@server.txt. Это окончательно подтверждает работоспособность двустороннего обмена данными (рис. [-@fig:022]).
