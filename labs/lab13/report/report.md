...
На клиентской машине выполняем команду sudo showmount -e server.nsandryushin.net, запрашивающую у NFS‑сервера список экспортируемых каталогов; в ответ отображается экспорт /srv/nfs для всех клиентов (*), что подтверждает доступность NFS‑сервера через настроенный межсетевой экран (рис. [-@fig:014]).

![Проверка экспорта NFS‑ресурса командой showmount](image/14.png){#fig:014}

На клиенте создаём каталог /mnt/nfs, после чего выполняем монтирование экспортируемого с сервера каталога server.nsandryushin.net:/srv/nfs в эту точку командой sudo mount; затем команда mount без параметров выводит список всех смонтированных файловых систем, в котором присутствует запись о подключённом ресурсе типа nfs4 с указанными параметрами монтирования, что подтверждает успешное подключение удалённого раздела (рис. [-@fig:015]).

![Монтирование каталога NFS на клиенте и проверка через mount](image/15.png){#fig:015}

Для настройки автоматического монтирования удаленного ресурса при запуске операционной системы отредактируем файл /etc/fstab на клиенте. В конец файла была добавлена запись server.nsandryushin.net:/srv/nfs /mnt/nfs nfs _netdev 0 0. Опция _netdev здесь указывает системе, что монтирование зависит от наличия сети, и предотвращает попытки монтирования до поднятия сетевых интерфейсов (рис. [-@fig:016]).
...
После перезагрузки клиента убедимся, что ресурс действительно подключен. Команда mount выводит список всех точек монтирования. В выводе присутствует строка, подтверждающая, что ресурс server.nsandryushin.net:/srv/nfs смонтирован в /mnt/nfs с типом файловой системы nfs4 (версия 4.2) и правами rw (чтение и запись), что соответствует успешному выполнению настройки (рис. [-@fig:018]).
...
На сервере создаём рабочий каталог common в домашней директории пользователя и создаём в нём тестовый файл. Затем создаём соответствующую директорию в дереве NFS (/srv/nfs/home/nsandryushin) и выполняем монтирование методом bind для объединения этих каталогов (рис. [-@fig:027]).
...
Для сохранения настроек монтирования после перезагрузки сервера вношу изменения в файл /etc/fstab. Добавляю записи для bind-монтирования каталогов /var/www и /home/nsandryushin/common в соответствующие точки монтирования дерева NFS (рис. [-@fig:029]).

![Настройка автоматического bind-монтирования в fstab](image/29.png){#fig:029}

После обновления таблицы экспорта на сервере (exportfs -r) перехожу на клиентскую машину и проверяю содержимое /mnt/nfs/. Вижу, что появилась директория home, а в ней — каталог пользователя nsandryushin (рис. [-@fig:030]).

![Проверка доступа к каталогу home на клиенте](image/30.png){#fig:030}

На клиенте в каталоге /mnt/nfs/home/nsandryushin создадим файл nsandryushin@client.txt с произвольным содержимым. При попытке выполнить действия в этом каталоге от имени суперпользователя получен отказ в доступе (Permission denied), что свидетельствует о корректной работе настройки безопасности NFS, которая по умолчанию запрещает удаленному root-пользователю иметь привилегии root на файловой системе сервера (рис. [-@fig:031]).

![Проверка доступа к смонтированному каталогу пользователя на клиенте](image/31.png){#fig:031}

На сервере проверим наличие созданного на клиенте файла. Файл nsandryushin@client.txt успешно отображается в каталоге /home/nsandryushin/common, содержимое файла совпадает с тем, что было записано на клиенте (рис. [-@fig:032]).
