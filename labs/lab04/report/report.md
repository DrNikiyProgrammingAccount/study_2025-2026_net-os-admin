...
![Файл зоны /var/named/master/fz/nsandryushin](image/20.png){#fig:020}
...
Теперь запустим dns службу и перейдём в папку с конфигурацией httpd (/etc/httpd/conf.d), создав дам файлы конфигурации для двух страниц - server.nsandryushin.net и www.nsandryushin.net (рис. [-@fig:023]).

![Создание конфигурационных файлов](image/23.png){#fig:023}

Поместим следующее содержимое в первый из файлов (рис. [-@fig:024]).

![server.nsandryushin.net](image/24.png){#fig:024}

И следующее содержимое во второй файл (рис. [-@fig:025]).

![www.nsandryushin.net](image/25.png){#fig:025}

В папке /var/www/html создадим папки server.nsandryushin.net и www.nsandryushin.net. В каждой из них создадим файл index.html, в каждый из которых запишем простую приветственную фразу (рис. [-@fig:026]).
...
Теперь с клиента попробуем перейти по адресу server.nsandryushin.net. Как видим, нам вывело страницу с той самой фразой, которую мы записывали в index.html ранее (рис. [-@fig:028]).

![server.nsandryushin.net](image/28.png){#fig:028}

Аналогичную ситуацию мы видим с сайтом по адресу www.nsandryushin.net (рис. [-@fig:029]).

![www.nsandryushin.net](image/29.png){#fig:029}
