...
Эта строка описывает программу, с помощью которой вы подключились. Это клиент mysql, который является частью дистрибутива MariaDB версии 10.11.11.   
...
Threads: 1 Questions: 27 Slow queries: 0 Opens: 20 Open tables: 13 Queries per second avg: 0.057   
...
0.057 — среднее количество запросов в секунду (рис. [-@fig:018]).
...
Теперь создадим пользователя nsandryushin и предоставим ему доступ к этой таблице, после чего убедимся, что количество баз данных увеличилось с 4 до 5. После этого попробуем показать базу данных addressbook от имени рута и от имени созданного нами пользователя. Как видим, операция успешна, а значит, пользователь имеет доступ к БД (рис. [-@fig:023]).
