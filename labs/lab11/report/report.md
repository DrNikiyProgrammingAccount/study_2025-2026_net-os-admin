...
Перезапустим службу sshd. Выводя статус службы можем усидеть, что служба не смогла привязаться к порту 2022 по причине нехватки прав доступа (рис. [-@fig:016]).

![Статус службы sshd](image/16.png){#fig:016}

Открыв вкладку с работающим journalctl мы можем увидеть, что проблема кроется в SElinux, который не даёт sshd использовать 2022 порт (рис. [-@fig:017]).

![Лог с SElinux](image/17.png){#fig:017}

Настроим SElinux метки так, чтобы разрешить sshd работать с 2022 портом и также настроим фаервол, разрешив пропускать трафик по 2022 порту, после чего перезапустим sshd и выведем его статус. Как видим, служба теперь прослушивает 2 порта - 22 и 2022 (рис. [-@fig:018]).

![Настройка SElinux и firewall](image/18.png){#fig:018}
...
В списке TCP служб мы увидим ssh, который перенаправляет трафик (рис. [-@fig:026]).

![Список служб](image/26.png){#fig:026}
