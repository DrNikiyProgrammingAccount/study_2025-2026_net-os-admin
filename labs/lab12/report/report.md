...
Далее проверим источники времени на клиентской машине после настройки конфигурации. Команда chronyc sources показывает список серверов. В выводе видно, что настроенный нами server.nsandryushin.net (помечен символом +) распознан как кандидат на синхронизацию, имеющий Stratum 3, что подтверждает корректность настройки DNS и доступность сервера (рис. [-@fig:015]).
